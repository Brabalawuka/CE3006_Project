...
encRandomInputBits = encode(randomInputBits,7,4,'hamming/binary');
...
for i = 0:10
    ...
    for j = 1:numberOfTrails
        ...
        decodedSignal = tansmitAndDemodulateFunction(sampledBPSK, signalVariance, SNR, 0.5, numberOfBits, sampleFreq, dataRate, carrierSingal,b,a);
        bitErrorRateTrails(j) = calculateBitError(randomInputBits, decodedSignal);

		encTransmittedSignal = tansmitAndDemodulateFunction(encSampledBPSK, signalVariance, SNR, 0.5, encNumberOfBits, sampleFreq, dataRate, encCarrierSingal,b,a);
        encDecodedSignal = decode(encTransmittedSignal,7,4,'hamming/binary');
...
% for i = 0:10
%     SNR = 5 * i;
%     numberOfTrails = 3; % For every SNR we transmit 100 times and take average to get errorrate
%     bitErrorRateTrails = zeros(numberOfTrails, 1);
% 	encBitErrorRateTrails = zeros(numberOfTrails, 1);
%     SNRAxis(i+1) = SNR;
%     for j = 1:numberOfTrails
...
hold on
title("Bit Error Rate vs SNR for OOK with and without Hamming coding");
legend({'y = OOK without Hamming coding','y= OOK with Hamming coding'},'Location','southeast')
xlabel('E_{b}/N_{0}') ;
ylabel('P_{e}') ;
