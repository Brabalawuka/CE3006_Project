...
encRandomInputBits = encode(randomInputBits,7,4,'cyclic/binary',genpoly);
...
for i = 0:10
    ...
    for j = 1:numberOfTrails
        ...
        encDecodedSignal = decode(encTransmittedSignal,7,4,'cyclic/binary',genpoly,trt);
...
hold on
title("Bit Error Rate vs SNR for OOK with and without cyclical coding");
legend({'y = AverageOOK','y= AverageOOKCyclical'},'Location','southeast')
xlabel('E_{b}/N_{0}') ;
ylabel('P_{e}') ;
